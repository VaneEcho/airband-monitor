...
def _iter_frames(args: argparse.Namespace) -> list[InferenceFrame] | object:
    if args.simulate:
    ...
    if args.input_wav_dir:
        ...
        return WavDirectorySource(Path(args.input_wav_dir), freq_mhz=args.wav_freq).read()
...
def _build_source_for_watch(args: argparse.Namespace) -> WavDirectorySource | RtlAirbandRecordingSource:
    if args.input_wav_dir:
        if args.wav_freq is None:
            raise ValueError("--wav-freq is required when using --input-wav-dir")
        return WavDirectorySource(Path(args.input_wav_dir), freq_mhz=args.wav_freq)
    if args.input_rtl_dir:
        return RtlAirbandRecordingSource(
            directory=Path(args.input_rtl_dir),
            default_freq_mhz=args.rtl_default_freq,
...
def _iter_new_watch_frames(args: argparse.Namespace, seen: set[str]) -> tuple[list[InferenceFrame], int, int]:
    source = _build_source_for_watch(args)
...
def main() -> None:
    ...
    if args.watch:
        ...
        loops = 0
        while True:
            frames, total_files, new_files = _iter_new_watch_frames(args, seen)
