...
def test_watch_only_returns_new_files(tmp_path: Path) -> None:
    ...
    frames1, total1, new1 = _iter_new_watch_frames(args, seen, HeuristicAudioClassifier())
    assert len(frames1) == 1
    assert total1 == 1
    assert new1 == 1

    frames2, total2, new2 = _iter_new_watch_frames(args, seen, HeuristicAudioClassifier())
    assert len(frames2) == 0
    assert total2 == 1
    assert new2 == 0

    _write_tone(tmp_path / "b.wav", 600.0)
    frames3, total3, new3 = _iter_new_watch_frames(args, seen, HeuristicAudioClassifier())
