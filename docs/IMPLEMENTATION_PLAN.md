...
## 8. RTL-SDR Blog V4 硬件特性备忘（R828D）

在实测中从官方文档整理的关键参数，直接影响采集质量：

| 特性 | 说明 |
|------|------|
| **增益可选值** | 29 档，0.0–49.6 dB；建议民航段 28–35 dB，超过 40 dB 后 FM 互调显著恶化 |
| **禁用 IF AGC** | V4 已知问题：开启 IF AGC 导致接收异常；必须手动设置增益（`-g 32`） |
| **DC 偏置尖峰** | 本振泄漏在中心频率形成强直流峰；用 `-E dc` 阻断，R828D 不支持 offset tuning |
| **采样率陷阱** | rtl_fm 实际输出率只能是内部率（约 1008000 Hz）的整数因子；非整数因子请求会被悄悄修改，导致 sox 速率不匹配、全零输出。安全值：168000（÷6）、144000（÷7）、201600（÷5） |
| **去加重（deemp）** | 仅适用于 FM 广播，民航 AM 不需要；误用会削掉语音高频 |
| **Bias-T** | 4.5V / 180mA max；每次重新插 USB 后需运行 `rtl_biast -d 0 -b 1`；带 LNA 不开则灵敏度严重下降 |
| **TCXO 精度** | 1 PPM（28.8 MHz 参考）；民航 25 kHz 信道间距下频率误差可忽略，无需额外 PPM 校准 |
| **直接采样（HF）** | V4 用内置 SA612 上变频器覆盖 HF（0–28 MHz），不用直接采样模式；24 MHz 以上自动切换为正常调谐，无需手动配置 |

**民航频段最大问题：FM 广播互调（IMD）**

RTL-SDR 动态范围有限，楼顶环境中强 FM 广播与本振非线性混合，在 118–137 MHz 内生成大量假峰，实测显示整个航空频段布满约 15–20 dB 的杂散信号。这些假信号位置固定，无音频内容，与真实航空 AM 载波的特征（突发、有话音包络）完全不同。

缓解方案（按优先级）：
1. **118–136 MHz 带通滤波器**（硬件，最有效，可减少 ~30 dB IMD）——生产部署必须
2. **降低增益**（28–32 dB，减轻前端压缩）
3. **信号特征区分**：真实信号为时域突发，假信号为持续存在；时序评分器的持续时间门控对此天然有效
...
| FM 广播互调在民航频段产生假信号 | **带通滤波器（生产必须）**；降增益至 28–32 dB；时序门控区分持续假信号与突发真信号 |
| 误报（分类器对噪声误判）| 保留全量录音日志，用可回放 clip 离线调阈值 |
